/**
 * DEPARTAMENTO MAKEUP & HAIR
 * C1 — Liquid Glass Design System
 * Continuidade de maquilhagem, penteados e caracterização
 */

import { useState, useMemo } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import {
  User, Sparkles, CheckCircle, Clock,
  Plus, X, Image,
} from 'lucide-react';
...
function ItemRow({ item, index }: { item: DeptItem; index: number }) {
  ...
  const name = item.name || item.notes?.substring(0, 40) || `Look ${item.id.substring(0, 6)}`;
  const deptLabel = item.department === 'hair' ? 'Cabelo' : 'Makeup';
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{
              ...
              background: 'rgba(168,85,247,0.15)', border: '0.5px solid rgba(168,85,247,0.3)',
              color: COLOR,
            }}>{deptLabel}</span>
          </div>
...
export function DepartamentoMakeupModule() {
  ...
  const items: DeptItem[] = useMemo(
    () => (departmentItems as DeptItem[]).filter(
      (i) => i.department === 'makeup' || i.department === 'hair'
    ),
    [departmentItems]
  );
  ...
  return (
    <LiquidPage
      title="Makeup & Hair"
      description="Continuidade de maquilhagem, penteados e caracterização"
      headerAction={
        <LiquidButton variant="purple" size="md" icon={<Plus size={16} />} onClick={() => setAddOpen(true)}>
          Novo Look
        </LiquidButton>
      }
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        <LiquidStatCard label="Personagens"    value={stats.uniqueChars} variant="purple"  icon={<User size={16} />}        animationDelay={0}   />
        <LiquidStatCard label="Looks definidos" value={stats.total}      variant="emerald" icon={<Sparkles size={16} />}   animationDelay={100} />
      ...
      <LiquidSection animated animationDelay={300}>
        {items.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '56px 0' }}>
            <Sparkles size={44} style={{ color: 'rgba(255,255,255,0.15)' }} />
            <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 14 }}>Sem items ainda</span>
            <LiquidButton variant="purple" size="sm" icon={<Plus size={14} />} onClick={() => setAddOpen(true)}>
            ...
            >
              ...
              <div className="relative z-10">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <h2 style={{ color: 'rgba(255,255,255,0.95)', fontSize: 17, fontWeight: 800 }}>Novo Look Makeup/Hair</h2>
                  <button
                ...
                <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 13 }}>
                  Use o módulo de Departamentos completo para gerir looks de maquilhagem e cabelo.
                </p>
...
DepartamentoMakeupModule.displayName = 'DepartamentoMakeupModule';
